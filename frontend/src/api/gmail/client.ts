...
export class GmailApiClient extends BaseApiClient {
  async getAuthUrl(): Promise<GmailAuthUrlResponse> {
    const response = await this.get<GmailAuthUrlResponse>('/api/integrations/gmail/auth-url');
    return response;
  }
  ...
  async sendEmail(data: SendEmailDto): Promise<SendEmailResponse> {
    const response = await this.post<SendEmailResponse>('/api/integrations/gmail/send-email', data);
    return response;
  }
}
